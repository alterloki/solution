...
public class ProblemB {
    ...
    private String solveCase(int k, int l, int s, String keyboard, String target) {
        ...
        if(possible) {
            ...
            long sum = findSum(keyboard, target, s);
            double count = Math.pow(keyboard.length(), s);
            System.out.println("c = " + count + " s = " + sum);
            result = max - (double)sum / count;
            return Double.toString(result);
        }
        return Double.toString(result);
    }

    private long findSum(String keyboard, String target, int s) {
        long result = 0;
        char[] str = new char[s];
